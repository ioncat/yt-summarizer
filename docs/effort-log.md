...
| 2026-05-17 | — | **Epic 22**: Auto language detection — `_detect_language()` + `_fetch_metadata()` two-call yt-dlp flow, "Auto (detect)" default in HomePage. **Epic 23**: Chapter-aware formatting — `Video.chapters` JSON column, `text_formatter.py` two branches (`_format_with_chapters()` vs `_format_with_gaps()`), `## Chapter Title` headings in formatted_text. **Epic 24**: Completion notifications — `notify()` tab title + Browser Notification on `processing → done` with `document.hidden` check. **Epic 25**: Heading preservation — `text_cleaner.py` bypasses `## ` paragraphs, prompts updated, `renderText()` in ResultPage renders `## ` as `<h3 class="chapter-heading">`. **Other**: Chat Q&A via `/api/chat` backend proxy (streaming, multi-turn), compression ratio in Summary meta, port centralization via `.env`. | ~4 h |
| 2026-05-17 | — | **Bug fix**: cancel was nulling `cleaned_text`/`summary_text` → fixed (`0500c2c`). **Epic 27 design + impl**: Full Extract (no-reduce) — `extract_notes()` in text_summarizer.py, `_split_by_chapter_headings()`, auto-select when `has_chapters AND len ≥ 24K`, `summary_mode = "full_extract"`, frontend meta + progress label. **Epic 26 design**: Benchmark — EPIC-26.md written, key principle "use same pipeline mode logic". **Epic 28 backlog**: Processing Mode Management. | ~2.5 h |
| 2026-05-17 | — | **Epic 26 impl** on `feature/epic-26-benchmark`: `BenchmarkRun` ORM model, `benchmark_service.py` (start_benchmark, _resolve_mode reusing _run_summary logic, parallel `asyncio.create_task`), API endpoints (POST /benchmark/run, GET /benchmark/{video_id}), `BenchmarkPage.tsx` with N-column CSS Grid + synchronized scroll + HTML export, `renderText()` extracted to shared util. **Epic 29 design + impl** on `feature/epic-29-parallel-map`: parallel MAP via `asyncio.Semaphore` + indexed-gather pattern in `clean_text()`, `_map_reduce()`, `extract_notes()`; `parallel_workers` app_setting; Settings UI field. **Docs**: USER_GUIDE.md (340 lines: Setup, Modes, Features, Settings ref, Troubleshooting, FAQ), README slimmed (174→110), Epic 21 deferred (duplicates Benchmark), Phase 3 epics renumbered 20-22 → 30-32 (collision fix), CLAUDE.md updated with Epic 25/27. | ~5 h |
