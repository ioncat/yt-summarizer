...
**Стратегическая оценка gap-ов**:
- **Поиск по транскрипту** — поверхностная фича, не создаёт ценность сама по себе. Ctrl+F в браузере решает это бесплатно.
- **Bulk** — инфраструктурная задача. Parallel workers у нас уже есть (`parallel_workers` setting). Очередь + API ключ = готово.
- **API** — у нас уже есть REST backend. Нужно только добавить auth + API keys. Не архитектурная проблема.
- **Образовательные инструменты** (Flash Cards, Quiz) — ниша. Легко добавить поверх сильного pipeline через дополнительные prompts.

**Вывод**: их gap-ы — косметика, добавляется быстро. Наш реальный приоритет — **глубина работы со смыслом**: pipeline, который действительно понимает структуру и содержание контента. Это сложнее скопировать.
